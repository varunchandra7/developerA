...
export async function seedDatabase(): Promise<void> {
  try {
    ...
    // Seed Herbs
    logger.info('🌿 Seeding herb data...');
    const createdHerbs = await HerbModel.insertMany(seedHerbs);
    logger.info(`✅ Created ${createdHerbs.length} herbs`);

    // Seed Compounds
    logger.info('🧪 Seeding compound data...');
    const createdCompounds = await Compound.insertMany(seedCompounds);
    logger.info(`✅ Created ${createdCompounds.length} compounds`);

    // Seed Literature
    logger.info('📚 Seeding literature data...');
    ...
    const createdLiterature = await Literature.insertMany(literatureWithAddedBy);
    logger.info(`✅ Created ${createdLiterature.length} literature entries`);

    // Seed Agents
    logger.info('🤖 Seeding agent data...');
    const createdAgents = await AgentModel.insertMany(seedAgents);
    ...
    logger.info(`   - Herbs: ${createdHerbs.length}`);
    logger.info(`   - Compounds: ${createdCompounds.length}`);
    logger.info(`   - Literature: ${createdLiterature.length}`);
    logger.info(`   - Agents: ${createdAgents.length}`);
...
async function createIndexes(): Promise<void> {
  // Herb indexes
  await HerbModel.collection.createIndex(
    { name: 'text', teluguName: 'text', sanskritName: 'text', scientificName: 'text' },
    { 
  ...
  await HerbModel.collection.createIndex({ 'ayurvedicProperties.dosha': 1 });
  await HerbModel.collection.createIndex({ 'modernProperties.activeCompounds': 1 });
  await HerbModel.collection.createIndex({ region: 1, status: 1 });

  // User indexes
  await UserModel.collection.createIndex({ email: 1 }, { unique: true });
  await UserModel.collection.createIndex({ role: 1, 'preferences.language': 1 });

  // Agent indexes
  await AgentModel.collection.createIndex({ agentId: 1 }, { unique: true });
  await AgentModel.collection.createIndex({ type: 1, status: 1 });
  await AgentModel.collection.createIndex({ 'metrics.successRate': -1 });
...
async function verifySeededData(): Promise<void> {
  ...
  const herbCount = await HerbModel.countDocuments();
  const compoundCount = await Compound.countDocuments();
  const literatureCount = await Literature.countDocuments();
  const agentCount = await AgentModel.countDocuments();

  logger.info(`🔍 Verification results:`);
  logger.info(`   - Users in database: ${userCount}`);
  logger.info(`   - Herbs in database: ${herbCount}`);
  logger.info(`   - Compounds in database: ${compoundCount}`);
  logger.info(`   - Literature entries in database: ${literatureCount}`);
  logger.info(`   - Agents in database: ${agentCount}`);

  // Test a complex query to ensure indexes work
  const searchResults = await HerbModel.find(
    { $text: { $search: 'turmeric inflammation' } },
    { score: { $meta: 'textScore' } }
  ).sort({ score: { $meta: 'textScore' } }).limit(5);

  logger.info(`🔍 Text search test returned ${searchResults.length} results`);

  if (searchResults.length > 0) {
    logger.info(`   - Top result: ${searchResults[0].name}`);
  }
}
...
export const seedFunctions = {
  ...
  seedHerbs: async () => {
    await HerbModel.insertMany(seedHerbs);
    logger.info(`✅ Seeded ${seedHerbs.length} herbs`);
  },
  
  seedCompounds: async () => {
    await Compound.insertMany(seedCompounds);
    logger.info(`✅ Seeded ${seedCompounds.length} compounds`);
  },
  
  seedLiterature: async () => {
    await Literature.insertMany(seedLiterature);
    logger.info(`✅ Seeded ${seedLiterature.length} literature entries`);
  },
  
  seedAgents: async () => {
    await AgentModel.insertMany(seedAgents);
    logger.info(`✅ Seeded ${seedAgents.length} agents`);
  },
