...
// Literature Schema
const LiteratureSchema = new mongoose_1.Schema({
    title: {
        type: String,
        required: true,
        trim: true,
...
LiteratureSchema.methods.calculateQualityScore = function () {
    let score = 0;
    // Peer review bonus
    if (this.quality.peerReviewed)
        score += 0.3;
    // Impact factor consideration
    if (this.quality.impactFactor) {
        if (this.quality.impactFactor > 10)
            score += 0.3;
        else if (this.quality.impactFactor > 5)
            score += 0.2;
        else if (this.quality.impactFactor > 2)
            score += 0.1;
    }
    // Citation count
    ...
    score += evidenceLevels[this.evidenceLevel] || 0;
    return Math.min(1, score);
};
LiteratureSchema.methods.getRelevanceForQuery = function (query, herbs = [], compounds = []) {
    let relevance = 0;
    const queryLower = query.toLowerCase();
    // Title and abstract relevance
    if (this.title.toLowerCase().includes(queryLower))
        relevance += 0.3;
    if (this.abstract && this.abstract.toLowerCase().includes(queryLower))
        relevance += 0.2;
    // Keyword relevance
    const keywordMatches = this.keywords.filter((k) => k.includes(queryLower) || queryLower.includes(k)).length;
    relevance += Math.min(0.2, keywordMatches * 0.05);
    // Herb relevance
    const herbMatches = herbs.filter((h) => this.focus.herbs.some((fh) => fh.includes(h.toLowerCase()))).length;
    relevance += Math.min(0.2, herbMatches * 0.1);
    // Compound relevance
    const compoundMatches = compounds.filter((c) => this.focus.compounds.some((fc) => fc.includes(c.toLowerCase()))).length;
...
LiteratureSchema.methods.extractKeyFindings = function () {
    ...
    if (this.extractedData?.clinicalFindings) {
        this.extractedData.clinicalFindings.forEach((finding) => {
            findings.push({
                type: 'clinical',
    ...
    if (this.extractedData?.compounds) {
        this.extractedData.compounds.forEach((compound) => {
            findings.push({
                type: 'compound',
                name: compound.name,
...
// Static methods
LiteratureSchema.statics.findByHerb = function (herbName) {
    return this.find({
        'focus.herbs': new RegExp(herbName, 'i'),
        status: 'active'
    }).sort({ relevanceScore: -1, 'quality.citationCount': -1 });
};
LiteratureSchema.statics.findByCompound = function (compoundName) {
    return this.find({
        'focus.compounds': new RegExp(compoundName, 'i'),
        status: 'active'
    }).sort({ relevanceScore: -1, 'quality.citationCount': -1 });
};
LiteratureSchema.statics.findByTherapeuticArea = function (area) {
    return this.find({
        'focus.therapeuticAreas': new RegExp(area, 'i'),
        status: 'active'
    }).sort({ evidenceLevel: 1, 'quality.citationCount': -1 });
};
LiteratureSchema.statics.findTraditionalTexts = function (tradition) {
    const query = {
...
LiteratureSchema.statics.searchLiterature = function (searchQuery, options = {}) {
    const query = {
    ...
    if (options.yearFrom || options.yearTo) {
        ...
        if (options.yearFrom)
            query['publication.year'].$gte = options.yearFrom;
        if (options.yearTo)
            query['publication.year'].$lte = options.yearTo;
    }
...
LiteratureSchema.pre('save', function (next) {
    ...
    this.relevanceScore = this.calculateQualityScore();
    // Ensure at least one author is marked as first author
    if (this.authors.length > 0 && !this.authors.some((a) => a.isFirstAuthor)) {
        this.authors[0].isFirstAuthor = true;
    }
    // Auto-extract keywords from title and abstract
    if (!this.keywords || this.keywords.length === 0) {
        this.keywords = this.extractKeywordsFromText();
    }
    next();
});
...
LiteratureSchema.methods.extractKeywordsFromText = function () {
    ...
    const relevantTerms = words.filter(word => word.length > 3 &&
...
// Export the model
exports.Literature = mongoose_1.default.model('Literature', LiteratureSchema);
exports.default = exports.Literature;
